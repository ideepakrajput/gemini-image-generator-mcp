...
async function generateImageWithGemini(prompt, model) {
    ...
    const inlineData = body?.candidates?.[0]?.content?.parts?.find((p) => p.inlineData)?.inlineData ??
        body?.candidates?.flatMap((c) => c?.content?.parts ?? []).find((p) => p.inlineData)?.inlineData;
    if (!inlineData?.data) {
...
export function createServer() {
    ...
    server.tool("generate_image", "Generate an image via Gemini and save to disk.", {
    ...
    server.tool("generate_and_upload", "Generate an image via Gemini, save it, then upload to upload-public endpoint.", {
        prompt: z.string().min(3),
        output_path: z.string().min(1),
        endpoint: z.string().url().optional(),
        bearer_token: z.string().optional(),
