...
function inferImageMimeType(name, fallback = "image/png") {
...
function bufferToBlobPart(data) {
...
function loadReferenceImageFromObject(input) {
...
function collectReferenceImageObjects(...values) {
    const result = [];
    for (const value of values) {
...
async function loadReferenceImageFromPath(imagePath) {
...
async function loadReferenceImageFromUrl(imageUrl) {
...
async function loadReferenceImages(input) {
    ...
    const encodedImages = collectStringList(input.reference_image_base64, input.reference_image_data_uri).map((data) => loadReferenceImageFromBase64({
        data,
        mimeType: input.reference_image_mime_type,
        name: input.reference_image_name
    }));
    const objectImages = collectReferenceImageObjects(input.reference_image, input.reference_images)
...
async function generateImageWithGemini(prompt, model, referenceImages = []) {
...
async function generateImageWithOpenAI(prompt, model, imageSize, referenceImages = []) {
    const apiKey = process.env.OPENAI_API_KEY?.trim();
    if (!apiKey) {
        throw new Error("OPENAI_API_KEY is not set.");
    }
    const base = process.env.OPENAI_BASE_URL?.trim()?.replace(/\/+$/, "") || "https://api.openai.com";
    const url = referenceImages.length > 0
    ...
    if (referenceImages.length > 0) {
        ...
        for (const image of referenceImages) {
            form.append(imageFieldName, new Blob([bufferToBlobPart(image.data)], { type: image.mimeType }), image.name);
        }
...
export function createServer() {
    ...
    server.tool("generate_image", "Generate an image via Gemini or OpenAI and save it to disk. Supports reference images from local paths, public URLs, base64/data URIs, or MCP-style image objects when a client such as Claude or Cursor exposes uploaded chat images to tool arguments.", sharedGenerationFields, async (input) => {
    ...
    server.tool("upload_image", "Upload a local image file to an upload-public endpoint (x-api-key supported).", {
    ...
    server.tool("generate_and_upload", "Generate an image via Gemini or OpenAI, save it, then upload it. Supports reference images from local paths, public URLs, base64/data URIs, or MCP-style image objects when a client such as Claude or Cursor exposes uploaded chat images to tool arguments.", {
